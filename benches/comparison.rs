...
fn do_xoshiro256plusplus(rng: &mut Xoshiro256PlusPlusX4, data: &mut U64x4) {
    for _ in 0..ITERATIONS {
        rng.next_u64x4(black_box(data));
    }
}
...
fn bench<M: Measurement, const T: u8>(c: &mut Criterion<M>) {
    ...
    let xoshiro245plusplus_name = format!("Xoshiro256PlusPlusX4 u64x4 - {suffix}");
    ...
    group.bench_function(xoshiro245plusplus_name, |b| {
        let mut rng: Xoshiro256PlusPlusX4 = Xoshiro256PlusPlusX4::seed_from_u64(0x0DDB1A5E5BAD5EEDu64);
        let mut data: U64x4 = Default::default();

        b.iter(|| do_xoshiro256plusplus(&mut rng, black_box(&mut data)))
    });
